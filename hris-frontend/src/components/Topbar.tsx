...
export default function Topbar({ title }: { title: string }) {
  ...
  // Logika untuk menentukan nama pengguna berdasarkan path
  const userName =
    pathname.startsWith("/user") // Memeriksa apakah path dimulai dengan '/user'
      ? "User"
      : pathname.startsWith("/admin") // Memeriksa apakah path dimulai dengan '/admin'
      ? "Admin"
      : "Guest";
